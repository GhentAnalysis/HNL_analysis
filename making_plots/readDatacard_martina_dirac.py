...
from drawPlot_martina import drawPlot
from drawPlot_martina import writeTables_sumErrors
...
if __name__ == '__main__':
    ...
    if flav[0] == 'muon':
        base_card_path = base_card_path.replace( '_mu_Dirac_muo', '_mu_Dirac_muo' )
        base_card_path2 = base_card_path2.replace( '_mu_Dirac_muo', '_mu_Dirac_muo' )
        base_card_path3 = base_card_path3.replace( '_mu_Dirac_muo', '_mu_Dirac_muo' )
        base_card_path4 = base_card_path4.replace( '_mu_Dirac_muo', '_mu_Dirac_muo' )
    if flav[0] == 'ele':
        base_card_path = base_card_path.replace( '_mu_Dirac_muo', '_e_Dirac_ele' )
        base_card_path2 = base_card_path2.replace( '_mu_Dirac_muo', '_e_Dirac_ele' )
        base_card_path3 = base_card_path3.replace( '_mu_Dirac_muo', '_e_Dirac_ele' )
        base_card_path4 = base_card_path4.replace( '_mu_Dirac_muo', '_e_Dirac_ele' )       
...
##################### central one! ##########################
    ...
    for i, card in enumerate( datacards ):  
        ...
        processCollections.append( ProcessCollection( process_list ) )   
        #merge the data
...
############ 2.3.4 ##############
    ...
    for i, card2 in enumerate( datacards2 ):  
        ...
        processCollections2.append( ProcessCollection( process_list2 ) )   
    ...
    for i, card3 in enumerate( datacards3 ):  
        ...
        processCollections3.append( ProcessCollection( process_list3 ) )   
    ...
    for i, card4 in enumerate( datacards4 ):  
        ...
        processCollections4.append( ProcessCollection( process_list4 ) )   
   ...
   #correct plot label for each variable:
    if len( plot ) == 1:
        if plot[0] == 'SR':
            ...
            plot_label = 'displacement'
        elif plot[0] == 'M3':
            plot_label = 'mass3'    
        else:
            print( 'Himmmmmm, A-Z analysis, money plot, Illia runs everything in one afternoon' )
    else:
        print( 'Himmmmmm, A-Z analysis, money plot, Illia runs everything in one afternoon' )
    ...
    if len( years ) == 1:
        if years[0] == '16':
            lumi_label = '35.9 fb^{-1} (13 TeV)'
        elif years[0] == '17':
            lumi_label = '41.5 fb^{-1} (13 TeV)'
        elif years[0] == '18':
            lumi_label = '59.7 fb^{-1} (13 TeV)'
        else:
            print( 'Himmmmmm, A-Z analysis, money plot, Illia runs everything in one afternoon' )

    elif len( years ) == 3:
        lumi_label = '137 fb^{-1} (13 TeV)'
    ...
    #legend_names = { 'DY' : 'DY', 'nonpromptDF' : 'nonprompt DF', 'nonpromptSF' : 'nonprompt SF', 'TTTX' : 'other', 'multiboson' : 'multiboson', 'Xgamma' : 'conversions' }
    #colors = { 'nonpromptDF' : ROOT.kGreen, 'nonpromptSF' : ROOT.kGreen + 2, 'TTTX' : ROOT.kRed-10, 'multiboson' : ROOT.kRed-10, 'Xgamma' : ROOT.kViolet+1 }

    #legend_names = { 'nonpromptDF' : 'nonprompt DF', 'nonpromptSF' : 'nonprompt SF', 'TTTX' : 'other', 'Xgamma' : 'conversions' }

    #new function to draw more than 1 signal

    
    drawPlot( total_data, total_process_collection,total_process_collection2,total_process_collection3, total_process_collection4, plot_name, colors, legend_names = legend_names, lumi_text = lumi_label, plot_axis= plot_label, flav_name = flav_label, mass1_name = mass1_label, mass2_name = mass2_label,mass3_name = mass3_label, v1_name = v1_label, v2_name = v2_label,v3_name = v3_label)
